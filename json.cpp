...
static int json_write_get_number_size(const struct json_number_s *number,
  ...
  if ((i < number->number_size) && ('I' == number->number[i])) {
    char *inf = const_cast<char*>("Infinity");
    size_t k;
...
static char *json_write_number(const struct json_number_s *number, char *data) {
  ...
  if ((i < number->number_size) && ('I' == number->number[i])) {
    ...
    if ('\0' == *inf++) {
      ...
      // Inf becomes 1.7976931348623158e308 because JSON can't support it
      for (dbl_max = const_cast<char*>("1.7976931348623158e308"); '\0' != *dbl_max; dbl_max++) {
        *data++ = *dbl_max;
      }
...
json::json(){rootowner=false;root=NULL;}

json::json(string jsonstring){
    root = json_parse(jsonstring.c_str(), strlen(jsonstring.c_str()));
    rootowner=true;  //parse the string
}

json::json(char* jsonstring){   //parse the string
  root = json_parse(jsonstring, strlen(jsonstring));
  rootowner=true;
}

json::json(json_value_s* rt){
  root = rt;
  rootowner=false;
}
...
json::~json(){
   if (rootowner) free(root);
}


void json::read(string jsonstring){
	if (rootowner && root) free(root);
	root = json_parse(jsonstring.c_str(), jsonstring.length());//parse the string
	rootowner=true;
}

void json::read(char* jsonstring){
	if (rootowner && root) free(root);
	root = json_parse(jsonstring, strlen(jsonstring));//parse the string
	rootowner=true;
}


struct json_object_element_s*   json::element(string target_name){
    struct json_object_s* object = (struct json_object_s*)root->payload;
    struct json_object_element_s* element = object->start;
    while(element && strcmp(element->name->string,target_name.c_str()))
        element = element->next;
    if (element==NULL)
        cout << "Json element \"" << target_name << "\" not found " << endl;
    return element;
}

//returns an object if value is an json_type_object otherwise nullstring
struct json_value_s* json::subobject(string  target_name){
   struct json_object_element_s* el =element(target_name);
   if (!el) return NULL;
   return (el->value);
}


string    json::stringvalue(string target_name){
    struct json_object_element_s* el =element(target_name);
    if (el->value->type!=json_type_string)
        cout << "Json element \"" << target_name << "\" not a string type " << endl;
    if (!el) return "" ;
    struct json_string_s* str= (struct json_string_s*)el->value->payload;
    return str->string;
}

float     json::numbervalue(string target_name){
    struct json_object_element_s* el =element(target_name);
    if (!el) return 0 ;
    if (el->value->type!=json_type_number)
        cout << "Json element \"" << target_name << "\" not a number type " << endl;
    struct json_string_s* str= (struct json_string_s*)el->value->payload;
    return strtof(str->string,NULL);
} //returns the numbervalue in case the value is an numberjson_type_number

bool  json::booleanvalue(string target_name){
    struct json_object_element_s* el =element(target_name);
    if (el->value->type==json_type_true) return true;
    if (el->value->type==json_type_false) return false;
    else
        cout << "Json element \"" << target_name << "\" not a boolean type " << endl;
    throw;
} //returns the booleanvalue in case the value is a boolean value

struct json_array_s* json::arrayvalue(string target_name){
    struct json_object_element_s* el =element(target_name);
    if (el->value->type==json_type_array)
        return  (struct json_array_s*)el->value->payload;
    else
        cout << "Json element \"" << target_name << "\" not a array type " << endl;
    throw;
} //returns the array in case the value is an numberjson_type_array


json_array::json_array(struct json_array_s* array_s){
    json_array_element_s *element;
    unsigned int nr_elements=array_s->length;
    objptrs=new vector<struct json_array_element_s*>(nr_elements);
    if (nr_elements)
    {   element= array_s->start;
        objptrs->at(0)=element;
    }
    for (unsigned int i=1;i<nr_elements;i++)
    {   element= element->next;
        objptrs->at(i)=element;
    }
};
json_array::~json_array(){
};

/*json_value_s* json_array::operator[](unsigned int index )
{  if (index<objptrs->size())
      return((*objptrs)[index]->value);
    cout <<"operator json::[] out of range: index= "<< index << " !"<<endl;
    exit(-1);
};*/
...
json_value_s* json_array::at(unsigned int index )
{  if (index<objptrs->size())
      return((*objptrs)[index]->value);
    cout <<"operator json::at() out of range: index= "<< index << " !"<<endl;
    exit(-1);
};
