...
#include <vector>
// vector needed in definition of C++ class

// this version is modified a little bit by B.van Leeuwen (2017-12) for easier use in C++ code. the class json however is definitely incomplete
// all modifications are made in the end of the file by addition of the class json
...
public:
    json();
    json(string jsonstring);  //parse the string
    json(char* jsonstring);  //parse the string
    json(json_value_s* r);  //give the root
    ~json();
    void		read(string jsonstring);
    void		read(char* jsonstring);
    inline bool loaded() {return root!=NULL;};

    struct json_value_s* subobject(string name);  // returns an object if value is an json_type_object otherwise nullstring
    string                stringvalue(string name);
    float                 numbervalue(string name); //returns the numbervalue in case the value is an numberjson_type_number
    bool                  booleanvalue(string target_name);
    struct json_array_s*  arrayvalue(string name);
private:
    struct json_value_s* root;
    struct json_object_element_s*   element(string target_name);
    bool   rootowner;
...
{
public:
    //json_array(struct json_array_s* array_s,unsigned int nr_elements);
    json_array(struct json_array_s* array_s);
    ~json_array();
    inline json_value_s* operator[](unsigned int index ){return at(index);};
    inline unsigned int size(){return (*objptrs).size();};
    json_value_s* at(unsigned int index );

private:
    vector<struct json_array_element_s*>	*objptrs;
};
